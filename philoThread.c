 ...
 * * * * * * * * * * * * * * * * */
# define _POSIX_SOURCE  // for rand_r
# include <stdio.h>
# include <stdlib.h>
...
{
  srand(time(NULL));
  for(int i=0; i<5; i++) {ph[i]=i; chopArray[i] = eatArr[5] = thinkArr[i] = 0;}  // set default values
  pthread_mutex_init(&mutex, NULL);
  pthread_t thread0, thread1, thread2, thread3, thread4;  // 5 threads. 1 per philosopher
  // initialize the thread and pass its values (philo number)
  if(pthread_create(&thread0, NULL, philoFunc, &ph[0])) {fprintf(stderr, "Error creating thread 0\n"); return 1;}
  if(pthread_create(&thread1, NULL, philoFunc, &ph[1])) {fprintf(stderr, "Error creating thread 1\n"); return 1;}
  if(pthread_create(&thread2, NULL, philoFunc, &ph[2])) {fprintf(stderr, "Error creating thread 2\n"); return 1;}
  if(pthread_create(&thread3, NULL, philoFunc, &ph[3])) {fprintf(stderr, "Error creating thread 3\n"); return 1;}
  if(pthread_create(&thread4, NULL, philoFunc, &ph[4])) {fprintf(stderr, "Error creating thread 4\n"); return 1;}

  if(pthread_join(thread0, NULL)) {fprintf(stderr, "Error joining thread 0\n"); return 2;}
  if(pthread_join(thread1, NULL)) {fprintf(stderr, "Error joining thread 1\n"); return 2;}
  if(pthread_join(thread2, NULL)) {fprintf(stderr, "Error joining thread 2\n"); return 2;}
  if(pthread_join(thread3, NULL)) {fprintf(stderr, "Error joining thread 3\n"); return 2;}
  if(pthread_join(thread4, NULL)) {fprintf(stderr, "Error joining thread 4\n"); return 2;}
